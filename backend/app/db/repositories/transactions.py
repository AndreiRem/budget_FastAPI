...
class TransactionRepository:
    ...
    async def get_by_id(self, transaction_id: int) -> TransactionResponse | None:
        result = await self.db.execute(select(TransactionORM).where(TransactionORM.id == transaction_id))
        return result.scalar_one_or_none()

    async def update(self, transaction_id: int, transaction_data: TransactionUpdate) -> TransactionResponse | None:
